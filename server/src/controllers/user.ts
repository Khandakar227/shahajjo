...
export const register = async (req: Request, res: Response) => {
  try {
    ...
    if (!mobileNo || !name)
      return res
        .status(400)
        .json({ error: true, message: "Missing mobile number or name" });

    let phoneNumber = parsePhoneNumber(mobileNo);
    if (!isPhoneNo(phoneNumber))
      return res
        .status(400)
        .json({ error: true, message: "Invalid mobile number" });
...
export const addDeviceToken = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(200)
      .json({ error: false, message: "Device token added successfully" });
  } catch (error) {
    const err = error as Error;
    console.log(err.message);
    res.status(500).json({
...
export const updateUserLocation = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(200)
      .json({ error: false, message: "Location updated successfully" });
  } catch (error) {
    const err = error as Error;
    console.log(err.message);
    res.status(500).json({
